...
class Smarty {
    ...
    public function __construct($config = []) {
        $default = [
            ...
            'tpl_end'      => '}',
            'view_path'    => '',
            'cache_path'   => Env::get('runtime_path') . 'temp' . DIRECTORY_SEPARATOR, // 模板缓存目录
            'cache_prefix' => '',
            'cache_suffix' => '.php',
            'tpl_dir'      => [Env::get('app_path') . 'public' . DIRECTORY_SEPARATOR . 'view'],
            'tpl_replace_string' => [],
        ];
    ...
    private function parseTemplate($template) {
        ...
        if (strpos($template, '@')) {
            ...
            $path = Env::get('app_path') . $module . DIRECTORY_SEPARATOR . 'view' . DIRECTORY_SEPARATOR;
        } else {
            // 当前视图目录
